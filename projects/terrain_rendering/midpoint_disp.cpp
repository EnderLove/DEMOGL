...
void MidpointDisp::DiamontStep(int rectSize, float curHeight){
...
void MidpointDisp::SquareStep(int rectSize, float curHeight){
    ...
    for (int y = 0; y < terrainSize_; y += rectSize){
        for (int x = 0; x < terrainSize_; x += rectSize){
            ...
            float curTopMid  = (curTopLeft + curCenter + curBotLeft + prevYCenter) / 4.0f + cpm::RandomFloatRange(-curHeight, curHeight);
